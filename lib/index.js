...
module.exports = (function() {
			...
			reader.read()
				.then(({ value, done }) => {
					...
					while (receivedData.includes('\n')) {
						const lineBreakIndex = receivedData.indexOf('\n');
						const line = receivedData.slice(0, lineBreakIndex);
						receivedData = receivedData.slice(lineBreakIndex + 1);
	...
	Serial.prototype.send = async function(data) {
		...
		return Promise.resolve().then(() => {
			return writer.write(dataEncoded)
				.then(() => {
	...
	// Function to send a JSON-RPC message to the serial port
	async function sendJsonRpcMessage(data) {
		const writer = await serialPort.writable.getWriter();
		const encoder = new TextEncoder();
		const jsonData = JSON.stringify(data);
		const dataEncoded = encoder.encode(jsonData); // Convert JSON data to Uint8Array

		return writer.write(dataEncoded)
		.then(() => {
			console.log('JSON-RPC message sent:', jsonData);
			writer.releaseLock();
		})
		.catch(error => {
	...
	Serial.prototype.sendJsonRpc = async function(method) {
	...
	Serial.prototype.connectToListen = async function(options) {
	...
	Serial.prototype.connect = function(options) {
		return Promise.resolve().then(() => {
			...
			return Promise.resolve().then(() => {
				...
				this.logger.log('Connecting EspLoader...');
			...
			if (options.listen) {
				return this.hardReset().then(() => {
					this.startListening();
					return new Promise((resolve, reject) => {
						try {
							const writeWaitingMessage = () => this.logger.log('Waiting for initial serial output from device...');
							const intervals = [
								setInterval(writeWaitingMessage, 2000),
								setInterval(() => this.logger.log('Try pressing BOOT button on device to reset manually.'), 5000),
							];
							const onInitialMessage = () => done();
							const done = () => {
								clearTimeout(timeout);
								intervals.forEach(interval => clearInterval(interval));
								this.removeListener('message', onInitialMessage);
								resolve();
							};
							const timeout = setTimeout(() => done(new Error('Timed-out while waiting for device serial output')), 30000);
							this.once('message', onInitialMessage);
							writeWaitingMessage();
						} catch (error) {
							return reject(error);
						}
	...
	Serial.prototype.disconnect = function() {
		return Promise.resolve().then(() => {
			const { loader } = this;
			if (loader) {
				this.logger.log('Disconnecting EspLoader...');
				this.stopListening();
				this.loader = null;
	...
	Serial.prototype.writePartitions = function(partitions) {
		return Promise.resolve().then(() => {
			...
			return this.connect({ listen: false, reconnect: true }).then(() => {
				const { loader } = this;
